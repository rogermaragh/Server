	...
	switch (settings.eCapturer)
	{
		...
		case Capturer::GDI:
			if (_strCapabilities.find("\"GDI\"") != std::string::npos)
			{
				...
				pScreenCapturer = new WinScreenCapture_GDI((char *)strDevice.c_str());
			}
		...
		case Capturer::GDIplus:
			if (_strCapabilities.find("\"GDI+\"") != std::string::npos)
			{
				...
				pScreenCapturer = new WinScreenCapture_GDIplus((char *)strDevice.c_str());
			}
			else LOG_ERROR("GDI+ capture not available!\n");
			break;
		case Capturer::D3D9:
			if (_strCapabilities.find("\"D3D9\"") != std::string::npos)
			{
				...
				pScreenCapturer = new WinScreenCapture_D3D9((char *)strDevice.c_str());
			}
		...
		case Capturer::D3D11:
			if (_strCapabilities.find("\"D3D11\"") != std::string::npos)
			{
				...
				pScreenCapturer = new WinScreenCapture_D3D11((char *)strDevice.c_str());
			}
		...
		case Capturer::RDP:
			if (_strCapabilities.find("\"RDP\"") != std::string::npos)
			{
				...
				pScreenCapturer = new WinScreenCapture_RDP((char *)strDevice.c_str());
			}
...
std::string WinScreenCaptureHelper::dumpDisplayDeviceInfo(const DISPLAY_DEVICE &displayDevice)
{
	...
	strString += "{ \"name\" : \"";
	strString += encodeURI((wchar_t *)displayDevice.DeviceName);
	strString += "\", \"description\" : \"";
	strString += encodeJSON((wchar_t *)displayDevice.DeviceString);
	strString += "\", \"flags\" : [";
